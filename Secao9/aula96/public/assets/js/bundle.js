...
var chkUpper = document.querySelector(".chk-maiusculas");
var chkLower = document.querySelector(".chk-minusculas");
var chkNums = document.querySelector(".chk-numeros");
var chkSymb = document.querySelector(".chk-simbolos");
...
function gera() {
  var senha = (0,_geradores__WEBPACK_IMPORTED_MODULE_0__.default)(qtdCaracteres.value, chkUpper.checked, chkLower.checked, chkNums.checked, chkSymb.checked);
...
function geraSenha(qtd, upper, lower, nums, symb) {
  if (!(qtd >= 0)) return;
  if (!upper && !lower && !nums && !symb) return;
  var senhaArray = [];
  qtd = Number(qtd);
  var loop = rand(rand(0, 4));
  var qtdGerada = 0;
